...
function renderContent(raw: string): string {
  ...
  for (const line of lines) {
    if (line.startsWith("```")) {
      if (!inCode) {
        ...
        out += `<div style="background:#09090b;border:1px solid #1c1c1f;padding:1.25rem 1.5rem;margin:1.25rem 0;overflow-x:auto">
          <pre style="margin:0;font-family:monospace;font-size:0.8125rem;line-height:1.75;color:#a1a1aa">${codeBuffer.trimEnd()}</pre>
        </div>`;
        codeBuffer = "";
      }
      continue;
    ...
    if (line.startsWith("### ")) {
      ...
      out += `<h3 id="${headingId(title)}" style="font-size:1rem;font-weight:700;color:#18181b;letter-spacing:-0.02em;margin:2rem 0 0.625rem">${title}</h3>`;
    } else if (line.startsWith("## ")) {
      flushList();
      const title = line.slice(3).trim();
      out += `<h2 id="${headingId(title)}" style="font-size:1.125rem;font-weight:700;color:#18181b;letter-spacing:-0.025em;margin:2.5rem 0 0.875rem;padding-bottom:0.625rem;border-bottom:1px solid #f4f4f5">${title}</h2>`;
    } else if (line.startsWith("# ")) {
      flushList();
      out += `<h1 style="font-size:1.5rem;font-weight:800;color:#18181b;letter-spacing:-0.03em;margin:0 0 1rem">${line.slice(2)}</h1>`;
...
export default class DocPage extends Component<PageProps<DocData>> {
  template({ data }: PageProps<DocData> = this.props) {
        ...
        return `
          <div style="margin-bottom:1.5rem">
            <div style="display:flex;align-items:center;gap:0.5rem;padding:0 0.75rem;margin-bottom:0.5rem">
              ...
              <p style="font-size:0.625rem;font-weight:700;text-transform:uppercase;letter-spacing:0.09em;color:#a1a1aa;margin:0">${CATEGORY_LABELS[cat] ?? cat}</p>
            </div>
            ${items}
          </div>
        `;
    ...
    const tableOfContents = headings.length
      ? headings
          .map(
            (item) => `<a href="#${item.id}" style="display:block;font-size:${item.level === 2 ? "0.75rem" : "0.7125rem"};padding:${item.level === 2 ? "0.35rem 0" : "0.25rem 0 0.25rem 0.7rem"};color:#71717a;text-decoration:none;line-height:1.5;transition:color 0.12s" onmouseover="this.style.color='#18181b'" onmouseout="this.style.color='#71717a'">${item.title}</a>`,
          )
          .join("")
      : "";
          ...
          <article class="doc-article">
            ...
            <div style="display:flex;align-items:center;gap:0.5rem;font-size:0.75rem;color:#a1a1aa;margin-bottom:2rem">
              ...
              <span style="display:inline-flex;align-items:center;gap:0.375rem">
                ...
                <span style="color:#71717a">${CATEGORY_LABELS[doc.category] ?? doc.category}</span>
              </span>
              <span style="color:#d4d4d8">/</span>
              <span style="color:#18181b;font-weight:500">${doc.title}</span>
            </div>

            <!-- Title -->
            <h1 style="font-size:clamp(1.5rem,3vw,2.25rem);font-weight:800;color:#18181b;letter-spacing:-0.04em;line-height:1.15;margin:0 0 1rem">${doc.title}</h1>
            <p style="font-size:1rem;color:#71717a;line-height:1.7;margin:0 0 2.5rem;max-width:56ch">${doc.excerpt}</p>
            ...
            <div style="margin-top:2rem;display:grid;grid-template-columns:1fr 1fr;gap:1px;background:#e4e4e7">
              ${prev
                ? `<a href="/docs/${prev.slug}"
                    ...
                    <span style="font-size:0.875rem;font-weight:600;color:#18181b">${prev.title}</span>
                  </a>`
              ...
              ${next
                ? `<a href="/docs/${next.slug}"
                    ...
                    <span style="font-size:0.875rem;font-weight:600;color:#18181b">${next.title}</span>
                  </a>`
                : `<div style="background:#fafafa"></div>`}
            </div>
